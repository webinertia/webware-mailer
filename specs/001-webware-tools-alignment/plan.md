...
`composer.lock`. This repository is the reference implementation; the plan is written so any
Webware package can execute it after filling in Package Parameters.
...
+ `mago.toml` extend), `webware/messagebus-event`

**Storage**: N/A (mailer has no database; reusable workflow DB steps skipped when `db-image`
omitted)
...
**Project Type**: PHP library (PSR interface + implementations, mailer with command bus and
...
**Scale/Scope**: 17 source classes; tooling alignment only (test scaffolding, not full coverage)
...
├── spec.md              # Feature spec (copy-into-project template)
├── plan.md              # This file
└── tasks.md             # Task list
```
...
test/
├── unit/                                  # WebwareTest\Mailer\
└── integration/                           # WebwareTestIntegration\Mailer\
```
...
Reference consumer is the canonical shape. Current package state compared file-by-file against the
reference (see spec's Package Parameters for mailer values).

### Phase 1 — composer.json

- `require.php`: `~8.4.1 || ~8.5.0`. Do not add `8.6.0-dev`.
- `require-dev`: PHPUnit `^12.5.33` → `^13.3.0`; add Infection `^0.34.1`, PHPBench `^1.7`,
  roave/backward-compatibility-check `^8.21.0`; keep `psr/http-*`, `webware/messagebus-event`,
  `webware/webware-tools`, `roave/security-advisories`.
- `config.platform.php`: `8.4.99`.
- `autoload-dev`: `WebwareTest\Mailer\` → `test/unit/`; add `WebwareTestIntegration\Mailer\` →
  `test/integration/`.
- `scripts`:
...
- `.github/workflows/continuous-integration.yml`: wrapper mirroring the reference with package
  ...
  - `with`: `php-versions: '["8.4", "8.5"]'`, `run-integration: true`, `enable-codecov: true`,
    ...
    `min-covered-msi: "95"`; omit DB inputs and `test-env-json`.
