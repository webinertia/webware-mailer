...
Tasks below are the copy-into-project execution list. Replace package parameters (PHP versions,
MSI thresholds, test namespaces) before executing in another package.
...
- [ ] T002 Align `require-dev`: PHPUnit `^13.3.0`, add `infection/infection: ^0.34.1`,
  `phpbench/phpbench: ^1.7`, `roave/backward-compatibility-check: ^8.21.0`; keep
  `psr/http-*`, `webware/messagebus-event`, `webware/webware-tools`,
...
- [ ] T018 Add README badges matching `webware/webware-message`: PHP version, latest version,
  ...
  URL embeds the branch segment (e.g. `.../webware-mailer/1.0.x`); update that segment in both the
  badge URL and the dashboard link whenever the default branch changes. Update `README.md`
